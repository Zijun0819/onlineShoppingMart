...
def index(request):
    ...
    category = TypeInfo.objects.get(id=1)
    category1 = typelist[0]
    goods_list = list(category.goodsinfo_set.all())  # 通过反向关系查询
    #  _set 连表操作
    type0 = typelist[0].goodsinfo_set.order_by('-gprice')[0:3]  # 按照上传顺序
    type01 = typelist[0].goodsinfo_set.order_by('-gprice')[0:3]  # 按照点击量
    type1 = typelist[1].goodsinfo_set.order_by('-gprice')[0:3]
    type11 = typelist[1].goodsinfo_set.order_by('-gprice')[0:3]
    cart_num = 0
    # 判断是否存在登录状态
